...
void ofApp::setup(){
  ...
  std::vector<ofxAudioData::ValiditySpec> sampleValiditySpecs {
    ...
    {ofxAudioAnalysisClient::AnalysisScalar::pitch, false, 20},
    {ofxAudioAnalysisClient::AnalysisScalar::pitch, true, 2000}
  };
...
void ofApp::update(){
  ...
  if (audioDataProcessorPtr->isDataValid()) {
    float s = audioDataProcessorPtr->getNormalisedScalarValue(ofxAudioAnalysisClient::AnalysisScalar::pitch, true); // false == not linear
